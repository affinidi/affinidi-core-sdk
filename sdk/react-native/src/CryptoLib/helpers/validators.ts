...
export function isValidKeyLength(length: number) {
  return !(length <= LENGTH_0 || length > MAX_KEY_LENGTH || parseInt(String(length)) !== length)
}
...
export function checkPublicKey(publicKey: Buffer): void {
  assert(
    publicKey.length === PREFIXED_DECOMPRESSED_LENGTH || publicKey.length === PREFIXED_KEY_LENGTH,
    ERROR_BAD_PUBLIC_KEY,
  )
