...
describe('CommonNetworkMember', () => {
  ...
  it('returns true when user is UNCONFIRMED', async () => {
    ...
    const isUnconfirmed = await AffinidiWallet.isUserUnconfirmed(username, options)
  ...
  it.skip('#generateCredentialOfferRequestToken, #verifyCredentialOfferResponseToken, #signCredentials, #validateCredential', async () => {
    ...
    const commonNetworkMemberIssuer = new AffinidiWallet(issuerPassword, issuerEncryptedSeed, options)
    const commonNetworkMemberHolder = new AffinidiWallet(holderPassword, holderEncryptedSeed, options)
  ...
  it('removes user if it is "UNCONFIMRED" before sign up', async () => {
    ...
    await AffinidiWallet.signUp(email, cognitoPassword, options)

    const token = await AffinidiWallet.signUp(email, cognitoPassword, options)
  ...
  it('#throws `COR-4 / 400` when UNCONFIRMED user login', async () => {
    ...
    await AffinidiWallet.signUp(username, cognitoPassword, options)

    let responseError

    try {
      await AffinidiWallet.fromLoginAndPassword(username, cognitoPassword, options)
    } catch (error) {
      responseError = error
    }
  ...
  it('#generateCredentialOfferRequestToken', async () => {
    ...
    const jwtObject = AffinidiWallet.fromJWT(tokenElem)
  ...
  it('#generateCredentialOfferRequestToken with JwtOptions', async () => {
    ...
    const tokenObject = AffinidiWallet.fromJWT(token)
  ...
  it('#validateCredential', async () => {
    ...
    const sucessStaticMethodResult = await AffinidiWallet.validateCredential(testPlatformTools, staticOptions as StaticValidateOptions, signedCredential as SignedCredential)
  ...
  it('#verifyCredentialOfferResponseToken', async () => {
    ...
    const tokenObject = AffinidiWallet.fromJWT(credentialOfferResponseToken)
  ...
  it('Auth flow', async () => {
    ...
    const requestTokenObject = AffinidiWallet.fromJWT(requestToken)
  ...
  it('#changeUsername throws `COR-7 / 409` when new username exists', async () => {
    ...
    const networkMember = await AffinidiWallet.fromLoginAndPassword(cognitoUsername, cognitoPassword, options)
  ...
  it('#signIn with phoneNumber returns token', async () => {
    const token = await AffinidiWallet.signIn(phoneNumber, options)
  ...
  it('#createWallet should return wallet with resolvable did (web)', async () => {
    const webDomain = 'did.actor:alice'
    ...
    expect(commonNetworkMember.didDocument).to.be.not.undefined
  ...
  it('#createWallet and #openWalletByEncryptedSeed should return the same did (elem)', async () => {
...

    {
      ...
      expect(cnmByCreateWallet.did).not.to.be.eql(cnmByOpenWalletByEncryptedSeed.did)
    }
  })
