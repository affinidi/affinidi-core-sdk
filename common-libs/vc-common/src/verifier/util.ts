...
export const genValidateFn = <T>(validations: Validations<T>): ValidateFn<T> => async (data) => {
...
export const isValid = <T>(validatorFn: ValidateFn<T>) => async (data: T): Promise<ValidatorResponse> => {
...
export const isTypeOf = (type: TypeOfTypes): Validator => (value) => {
  return createValidatorResponse(typeof value === type, `Expected to be typeof: "${type}"`)
}

export const isUndefinedOr = <T>(validator: Validator<T>): Validator<T> => async (value, data) => {
  if (typeof value === 'undefined') return true
  return validator(value, data)
}

export const isArrayOf = <T>(validator: Validator<T>, rejectEmpty = true): Validator<T> => async (value, data) => {
...
export const isOneOf = <T>(...validators: Validator<T>[]): Validator<T> => async (value, data) => {
